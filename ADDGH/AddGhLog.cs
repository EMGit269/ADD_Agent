    ...
    public static class AddGhLog
    {
        ...
        private static string LogDirectory
        {
            get
            {
                ...
                _logDir = Path.Combine(root, "ADDGH", "logs");
        ...
        private static string LogFilePath =>
            Path.Combine(LogDirectory, "addgh-" + DateTime.Now.ToString("yyyyMMdd") + ".log");
        ...
        public static void Error(string message, Exception ex = null)
        {
            ...
            MaybeShowTemporaryPopup("ADDGH — 错误", m, MessageBoxIcon.Error);
        }

        /// <summary>
        /// 临时排错：在启用 ADDGH_ERROR_POPUP=1 时弹窗提示（并写入 Warning 日志行）。
        /// </summary>
        public static void UserAlert(string title, string message)
        {
            title = string.IsNullOrWhiteSpace(title) ? "ADDGH" : title.Trim();
            string line = message ?? "";
            Write(AddGhLogLevel.Warning, "UserAlert: " + title + " — " + line);
            MaybeShowTemporaryPopup(title, line, MessageBoxIcon.Warning);
        }

        /// <summary> 仅 getenv ADDGH_ERROR_POPUP=1 时弹出；避免常驻打扰。 </summary>
        internal static void MaybeShowTemporaryPopup(string title, string text, MessageBoxIcon icon = MessageBoxIcon.Exclamation)
        {
            ...
            void ShowSafe()
            {
                try
                {
                    MessageBox.Show(text ?? "", title ?? "ADDGH", MessageBoxButtons.OK, icon,
        ...
        private static void Write(AddGhLogLevel level, string message)
        {
            ...
            System.Diagnostics.Debug.WriteLine("[ADDGH] " + line.TrimEnd());
