...
9. Starting with the second C# Script component in a solution, avoid dense inter-script wiring: if the script has many ports, connect only the 1-2 ports that must receive upstream data and set the other ports through safe defaults or local helper inputs.
...
- Match input names and port order exactly. For outputs, assign values to the requested output variable names declared for the C# Script.
...
- For the second and later C# Script components, prefer sparse connections between scripts. Do not wire every available upstream value into every downstream script; connect the essential 1-2 data ports and keep secondary parameters local/defaulted.
...
- when more than one C# Script is planned, state which 1-2 ports are actually connected between scripts and which ports use defaults/local helpers
